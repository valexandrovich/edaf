import axios from 'axios'
import { i18n } from '../plugins/i18n'

// http: //localhost:1337/api/products?filters[productType][$eq]=artistTalks

const API_URL = process.env.VUE_APP_API_BASE + '/api/'

class ProductService {
    getProductById(id){
        return axios.get(API_URL + 'products/' + id, {
            headers: {
                'Authorization': 'bearer '
            }
        })


    }
    getProductSpaceById(id){
        return axios.get(API_URL + 'product-spaces/' + id, {
            headers: {
                'Authorization': 'bearer '
            }
        })


    }
    getProductPartnerById(id){
        return axios.get(API_URL + 'product-partners/' + id, {
            headers: {
                'Authorization': 'bearer '
            }
        })


    }
    getProducts() {
        let locale = i18n.locale
            // ?pagination[start]=0&pagination[limit]=100
        return axios.get(API_URL + 'products?populate=*&locale=' + locale + '&pagination[start]=0&pagination[limit]=100', {
            headers: {
                'Authorization': 'bearer '
            }
        })
    }
    getProductSpaces() {
        let locale = i18n.locale
            // ?pagination[start]=0&pagination[limit]=100
        return axios.get(API_URL + 'product-spaces?populate=*&locale=' + locale + '&pagination[start]=0&pagination[limit]=100', {
            headers: {
                'Authorization': 'bearer '
            }
        })
    }
    getProductPartners() {
        let locale = i18n.locale
            // ?pagination[start]=0&pagination[limit]=100
        return axios.get(API_URL + 'product-partners?populate=*&locale=' + locale + '&pagination[start]=0&pagination[limit]=100', {
            headers: {
                'Authorization': 'bearer '
            }
        })
    }
}

export default new ProductService()